...
const createOfferedCourseIntoDB = async (payload: TOfferedCourse) => {
    const { semesterRegistration, academicDepartment, academicFaculty, course, section, faculty, days, startTime, endTime } = payload;
     ...
     * 6. check if the dept belong to the faculty
     * 7. check duplicate possibility of offered course of same section in same registered semester
     * 8. check the schedule of faculty
     * 9. check the availability of faculty according to schedule
     * 10. create offered course 
    */

    // step - 1
    const doesSemesterRegistrationExist = await SemesterRegistration.findById(semesterRegistration);
    ...
    // step - 6
    const doesDepartmentBelongToFaculty = await AcademicDepartment.findOne({ _id: academicDepartment, academicFaculty });

    if (!doesDepartmentBelongToFaculty) {
        throw new AppError(httpStatus.BAD_REQUEST, `Dept : ${doesAcademicDepartmentExits.name} does not belong to faculty : ${doesFacultyExits.name}`)
    }

    // step - 7
    const doesSameOfferedCourseExistWithSameRegisteredSemesterAndSameSection = await OfferedCourse.findOne({
...
const updateOfferedCourseInDB = async (id: string, payload: Pick<TOfferedCourse, 'faculty' | 'days' | 'startTime' | 'endTime'>) => {
    /**
    ...
    */
    ...
    const registeredSemester = await SemesterRegistration.findById(registeredSemesterId);
...
export const OfferedCourseServices = {
    createOfferedCourseIntoDB, getAllOfferedCoursesFromDB, getSingleOfferedCourseFromDB, deleteOfferedCourseFromDB, updateOfferedCourseInDB
}